...
export async function pegarTransacoesPorId (req, res) {
    ...
    try {
        ...
        if (!atualizar) return res.status(404).send("Essa transação é inexistente!");
        res.send(atualizar);
    } catch (err) {
        res.status(500).send(err.message);
    }
...
export async function deletaTransacao (req, res) {
	...
	try {
		...
		if (result.deletedCount === 0) return res.status(404).send("Transação não existe!");
...
export async function editaTransacao (req, res) {
	...
	try {
		...
		if (result.matchedCount === 0) return res.status(404).send("Essa transação é inexistente!");

		res.send("Transação está atualizada!");
